...
class FrogPilotVariables:
  ...
  def update_frogpilot_params(self, started):
    ...
    toggle.force_auto_tune = advanced_lateral_tune and not has_auto_tune and not is_pid_car and self.params.get_bool("ForceAutoTune")
    ...
    advanced_longitudinal_tune = openpilot_longitudinal and self.params.get_bool("LongitudinalTune")
    toggle.lead_detection_threshold = clip(self.params.get_int("LeadDetectionThreshold") / 100, 0.01, 0.99) if advanced_longitudinal_tune else 0.5
    toggle.max_desired_accel = clip(self.params.get_float("MaxDesiredAcceleration"), 0.1, 4.0) if advanced_longitudinal_tune else 4.0
    ...
    curve_speed_controller = openpilot_longitudinal and self.params.get_bool("CurveSpeedControl")
    ...
    toggle.aggressive_jerk_acceleration = clip(self.params.get_int("AggressiveJerkAcceleration") / 100, 0.01, 5) if aggressive_profile else 0.5
    toggle.aggressive_jerk_deceleration = clip(self.params.get_int("AggressiveJerkDeceleration") / 100, 0.01, 5) if aggressive_profile else 0.5
    toggle.aggressive_jerk_danger = clip(self.params.get_int("AggressiveJerkDanger") / 100, 0.01, 5) if aggressive_profile else 0.5
    toggle.aggressive_jerk_speed = clip(self.params.get_int("AggressiveJerkSpeed") / 100, 0.01, 5) if aggressive_profile else 0.5
    toggle.aggressive_jerk_speed_decrease = clip(self.params.get_int("AggressiveJerkSpeedDecrease") / 100, 0.01, 5) if aggressive_profile else 0.5
    toggle.aggressive_follow = clip(self.params.get_float("AggressiveFollow"), 1, 5) if aggressive_profile else 1.25
    standard_profile = toggle.custom_personalities and self.params.get_bool("StandardPersonalityProfile")
    toggle.standard_jerk_acceleration = clip(self.params.get_int("StandardJerkAcceleration") / 100, 0.01, 5) if standard_profile else 1.0
    toggle.standard_jerk_deceleration = clip(self.params.get_int("StandardJerkDeceleration") / 100, 0.01, 5) if standard_profile else 1.0
    toggle.standard_jerk_danger = clip(self.params.get_int("StandardJerkDanger") / 100, 0.01, 5) if standard_profile else 0.5
    toggle.standard_jerk_speed = clip(self.params.get_int("StandardJerkSpeed") / 100, 0.01, 5) if standard_profile else 1.0
    toggle.standard_jerk_speed_decrease = clip(self.params.get_int("StandardJerkSpeedDecrease") / 100, 0.01, 5) if standard_profile else 1.0
    toggle.standard_follow = clip(self.params.get_float("StandardFollow"), 1, 5) if standard_profile else 1.45
    relaxed_profile = toggle.custom_personalities and self.params.get_bool("RelaxedPersonalityProfile")
    toggle.relaxed_jerk_acceleration = clip(self.params.get_int("RelaxedJerkAcceleration") / 100, 0.01, 5) if relaxed_profile else 1.0
    toggle.relaxed_jerk_deceleration = clip(self.params.get_int("RelaxedJerkDeceleration") / 100, 0.01, 5) if relaxed_profile else 1.0
    toggle.relaxed_jerk_danger = clip(self.params.get_int("RelaxedJerkDanger") / 100, 0.01, 5) if relaxed_profile else 0.5
    toggle.relaxed_jerk_speed = clip(self.params.get_int("RelaxedJerkSpeed") / 100, 0.01, 5) if relaxed_profile else 1.0
    toggle.relaxed_jerk_speed_decrease = clip(self.params.get_int("RelaxedJerkSpeedDecrease") / 100, 0.01, 5) if relaxed_profile else 1.0
    toggle.relaxed_follow = clip(self.params.get_float("RelaxedFollow"), 1, 5) if relaxed_profile else 1.75
    traffic_profile = toggle.custom_personalities and self.params.get_bool("TrafficPersonalityProfile")
    toggle.traffic_mode_jerk_acceleration = [clip(self.params.get_int("TrafficJerkAcceleration") / 100, 0.01, 5), toggle.aggressive_jerk_acceleration] if traffic_profile else [0.5, 0.5]
    toggle.traffic_mode_jerk_deceleration = [clip(self.params.get_int("TrafficJerkDeceleration") / 100, 0.01, 5), toggle.aggressive_jerk_deceleration] if traffic_profile else [0.5, 0.5]
    toggle.traffic_mode_jerk_danger = [clip(self.params.get_int("TrafficJerkDanger") / 100, 0.01, 5), toggle.aggressive_jerk_danger] if traffic_profile else [1.0, 1.0]
    toggle.traffic_mode_jerk_speed = [clip(self.params.get_int("TrafficJerkSpeed") / 100, 0.01, 5), toggle.aggressive_jerk_speed] if traffic_profile else [0.5, 0.5]
    toggle.traffic_mode_jerk_speed_decrease = [clip(self.params.get_int("TrafficJerkSpeedDecrease") / 100, 0.01, 5), toggle.aggressive_jerk_speed_decrease] if traffic_profile else [0.5, 0.5]
    toggle.traffic_mode_t_follow = [clip(self.params.get_float("TrafficFollow"), 0.5, 5), toggle.aggressive_follow] if traffic_profile else [0.5, 1.0]

    custom_paths = self.params.get_bool("CustomUI") and self.params.get_bool("CustomPaths")
    ...
    toggle.increase_thermal_limits = device_management and self.params.get_bool("IncreaseThermalLimits")
    toggle.low_voltage_shutdown = clip(self.params.get_float("LowVoltageShutdown"), VBATT_PAUSE_CHARGING, 12.5) if device_management else VBATT_PAUSE_CHARGING
    toggle.offline_mode = device_management and self.params.get_bool("OfflineMode")
    ...
    model_manager = self.params.get_bool("ModelManagement", block=openpilot_installed)
    available_models = self.params.get("AvailableModels", block=model_manager, encoding='utf-8') or ""
    available_model_names = self.params.get("AvailableModelsNames", block=model_manager, encoding='utf-8') or ""
    if model_manager and available_models:
    ...
    self.params.put("FrogPilotToggles", pickle.dumps(toggle))
    self.params_memory.remove("FrogPilotTogglesUpdated")

  def publish_frogpilot_params(self, pm):
    frogpilot_toggles_send = messaging.new_message('frogpilotToggles')
    frogpilot_toggles_send.frogpilotToggles.frogpilotToggles = [json.dumps(vars(self.frogpilot_toggles))]
    pm.send('frogpilotToggles', frogpilot_toggles_send)
