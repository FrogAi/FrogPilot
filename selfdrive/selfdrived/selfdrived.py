...
class SelfdriveD:
  def __init__(self, CP=None):
    ...
    # FrogPilot variables
    self.sm = self.sm.extend(['frogpilotCarState', 'frogpilotPlan', 'frogpilotUI'])
  ...
  def update_events(self, CS):
    ...
    for i, pandaState in enumerate(self.sm['pandaStates']):
      ...
      if i < len(self.CP.safetyConfigs):
        safety_mismatch = pandaState.safetyModel != self.CP.safetyConfigs[i].safetyModel or \
                          pandaState.safetyParam != (self.CP.safetyConfigs[i].safetyParam | self.FPCP.safetyConfigs[i].safetyParam) or \
                          pandaState.alternativeExperience != (self.CP.alternativeExperience | self.FPCP.alternativeExperience)
      else:
    ...
    has_disable_events = frogpilot_utilities.contains_event_type(self.events, self.frogpilot_events, ET.NO_ENTRY) and \
                         (frogpilot_utilities.contains_event_type(self.events, self.frogpilot_events, ET.SOFT_DISABLE) or
                          frogpilot_utilities.contains_event_type(self.events, self.frogpilot_events, ET.IMMEDIATE_DISABLE))
