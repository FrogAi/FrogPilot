...
static void gm_rx_hook(const CANPacket_t *msg) {
  ...
  if (msg->bus == 0U) {
    ...
    // OPGM variables
    // No-ACC cruise state from ECMCruiseControl
...
static bool gm_tx_hook(const CANPacket_t *msg) {
  ...
  // OPGM variables
  // Raw command tracks produced by gas_amount == 1.0 in create_gas_interceptor_command.
  const int GM_MAX_INTERCEPTOR_TRACK_1 = 2633;
  const int GM_MAX_INTERCEPTOR_TRACK_2 = 1316;
  ...
  if (msg->addr == 0x1E1U) {
    if (gm_pcm_cruise || gm_pedal_long || gm_cc_long) {
      ...
      if (gm_cc_long) {
        allowed_btn |= controls_allowed && cruise_engaged_prev &&
  ...
  if (msg->addr == 0x200U) {
    if (!enable_gas_interceptor || longitudinal_interceptor_checks(msg, GM_MAX_INTERCEPTOR_TRACK_1, GM_MAX_INTERCEPTOR_TRACK_2)) {
      tx = false;
    }
  }
...
static safety_config gm_init(uint16_t param) {
  ...
  // OPGM variables
  static const CanMsg GM_CAM_LONG_INTERCEPTOR_TX_MSGS[] = {{0x180, 0, 4, .check_relay = true}, {0x315, 0, 5, .check_relay = true}, {0x2CB, 0, 8, .check_relay = true}, {0x370, 0, 6, .check_relay = true},  // pt bus
                                                           {0x184, 2, 8, .check_relay = true},  // camera bus
                                                           {0x200, 0, 6, .check_relay = false}};  // pt bus

  static const CanMsg GM_CAM_LONG_PEDAL_TX_MSGS[] = {{0x180, 0, 4, .check_relay = true}, {0x315, 0, 5, .check_relay = true}, {0x2CB, 0, 8, .check_relay = true}, {0x370, 0, 6, .check_relay = true},  // pt bus
                                                      {0x184, 2, 8, .check_relay = true},  // camera bus
  ...
  // OPGM variables
  static const CanMsg GM_CAM_PEDAL_TX_MSGS[] = {{0x180, 0, 4, .check_relay = true},  // pt bus
                                                {0x184, 2, 8, .check_relay = true},  // camera bus
                                                {0x200, 0, 6, .check_relay = false}, {0x1E1, 0, 7, .check_relay = false}};  // pt bus
  ...
  static RxCheck gm_no_acc_ev_rx_checks[] = {
    GM_COMMON_RX_CHECKS
    {.msg = {{0xBD, 0, 7, 40U, .ignore_checksum = true, .ignore_counter = true, .ignore_quality_flag = true}, { 0 }, { 0 }}},
    {.msg = {{0x3D1, 0, 8, 10U, .ignore_checksum = true, .ignore_counter = true, .ignore_quality_flag = true}, { 0 }, { 0 }}},  // no-ACC cruise status
  };
  ...
  static RxCheck gm_acc_pedal_rx_checks[] = {
    GM_COMMON_RX_CHECKS
    GM_ACC_RX_CHECKS
    {.msg = {{0x201, 0, 6, 10U, .ignore_checksum = true, .ignore_counter = true, .ignore_quality_flag = true}, { 0 }, { 0 }}},  // gas interceptor sensor
  };

  static RxCheck gm_acc_ev_pedal_rx_checks[] = {
    GM_COMMON_RX_CHECKS
    {.msg = {{0xBD, 0, 7, 40U, .ignore_checksum = true, .ignore_counter = true, .ignore_quality_flag = true}, { 0 }, { 0 }}},
    GM_ACC_RX_CHECKS
    {.msg = {{0x201, 0, 6, 10U, .ignore_checksum = true, .ignore_counter = true, .ignore_quality_flag = true}, { 0 }, { 0 }}},  // gas interceptor sensor
  ...
  static RxCheck gm_no_acc_pedal_rx_checks[] = {
    GM_COMMON_RX_CHECKS
    {.msg = {{0x3D1, 0, 8, 10U, .ignore_checksum = true, .ignore_counter = true, .ignore_quality_flag = true}, { 0 }, { 0 }}},  // no-ACC cruise status
    {.msg = {{0x201, 0, 6, 10U, .ignore_checksum = true, .ignore_counter = true, .ignore_quality_flag = true}, { 0 }, { 0 }}},  // gas interceptor sensor
  };

  static RxCheck gm_no_acc_ev_pedal_rx_checks[] = {
    GM_COMMON_RX_CHECKS
    {.msg = {{0xBD, 0, 7, 40U, .ignore_checksum = true, .ignore_counter = true, .ignore_quality_flag = true}, { 0 }, { 0 }}},
    {.msg = {{0x3D1, 0, 8, 10U, .ignore_checksum = true, .ignore_counter = true, .ignore_quality_flag = true}, { 0 }, { 0 }}},  // no-ACC cruise status
    {.msg = {{0x201, 0, 6, 10U, .ignore_checksum = true, .ignore_counter = true, .ignore_quality_flag = true}, { 0 }, { 0 }}},  // gas interceptor sensor
  ...
  const bool gm_pedal_interceptor = GET_FLAG(param, GM_PARAM_PEDAL_INTERCEPTOR) && (gm_hw == GM_CAM);

  const uint16_t GM_PARAM_NO_ACC = 32;
  gm_has_acc = !GET_FLAG(param, GM_PARAM_NO_ACC);

  const uint16_t GM_PARAM_PEDAL_LONG = 64;
  gm_pedal_long = gm_pedal_interceptor && GET_FLAG(param, GM_PARAM_PEDAL_LONG);
  enable_gas_interceptor = gm_pedal_interceptor && (gm_cam_long || gm_pedal_long);
  ...
  if (gm_hw == GM_CAM) {
    ...
    } else if (gm_cam_long && gm_pedal_long) {
      ret = BUILD_SAFETY_CFG(gm_rx_checks, GM_CAM_LONG_PEDAL_TX_MSGS);
    } else if (gm_cam_long && enable_gas_interceptor) {
      ret = BUILD_SAFETY_CFG(gm_rx_checks, GM_CAM_LONG_INTERCEPTOR_TX_MSGS);
    } else if (gm_cam_long) {
      ret = BUILD_SAFETY_CFG(gm_rx_checks, GM_CAM_LONG_TX_MSGS);
    } else if (gm_pedal_long) {
      ret = BUILD_SAFETY_CFG(gm_rx_checks, GM_CAM_PEDAL_TX_MSGS);
    } else {
  ...
  if (enable_gas_interceptor && !gm_has_acc && gm_ev) {
    SET_RX_CHECKS(gm_no_acc_ev_pedal_rx_checks, ret);
  } else if (enable_gas_interceptor && !gm_has_acc && !gm_ev) {
    SET_RX_CHECKS(gm_no_acc_pedal_rx_checks, ret);
  } else if (enable_gas_interceptor && gm_ev) {
    SET_RX_CHECKS(gm_acc_ev_pedal_rx_checks, ret);
  } else if (enable_gas_interceptor) {
    SET_RX_CHECKS(gm_acc_pedal_rx_checks, ret);
