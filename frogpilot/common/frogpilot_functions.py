...
def backup_frogpilot(build_metadata, params):
  ...
  if free > minimum_backup_size * maximum_backups:
    ...
    backup_directory(directory, destination_directory, f"Successfully backed up FrogPilot to {destination_directory}", f"Failed to backup FrogPilot to {destination_directory}", params, minimum_backup_size, compressed=True)
...
def backup_toggles(params, params_cache):
  ...
  source_directory = Path(params_backup.get_param_path())

  backup_directory(source_directory, destination_directory, f"Successfully backed up toggles to {destination_directory}", f"Failed to backup toggles to {destination_directory}", params)
